...
static void	ft_in_pb_pa(t_comand_list **comands, int count)
{
	int				i;
	t_comand_list	*tmp;
	...
	if (count > 0)
	{
		while (ft_strequ((*comands)->comand, "pb\n") == 1)
...
void		ft_remove_pb_pa(t_comand_list **comands)
{
	...
	{
		pb++;
		if (com->next && ft_strequ(com->next->comand, "pa\n") == 1)
		{
			com = com->next;
